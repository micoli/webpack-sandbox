	...
	for (let y = 0; y < height; y++) {
		for (let x = 0; x < width; x++) {
			value=(
				...
				(openSimplex.noise2D(offsetX/16+x/16, offsetY/16+y/16) + 0.7) * 1 +
				+(openSimplex.noise2D(offsetX/8 +x/8, offsetY/8+y/8) + 0.7) * 0.5
			)/(1.5*2)
			data.push(value);
		}
	}
...
function drawterrain(data:number[]){
	...
	for (let x = 0; x < width; x++) {
		for (let y = 0; y < height; y++) {
			...
			imageData.data[idx + 0] = value>0?value*255:255;
			imageData.data[idx + 1] = value>0?value*255:255;
			imageData.data[idx + 2] = value>0?value*255:0;
